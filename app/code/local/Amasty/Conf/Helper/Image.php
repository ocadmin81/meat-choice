...
class Amasty_Conf_Helper_Image extends Mage_Core_Helper_Abstract
{
    ...
    public function init($img = '')
    {
        ...
        {
            $this->placeHolder = Mage::getBaseUrl('media') . 'amconf' . '/' . 'images' . '/0.jpg';
        }
    ...
    public function imagePath($img='')
    {
        ...
        $img = str_replace($this->getSiteUrl(), '/', $img);
    ...
    {
        $media = str_replace($this->getSiteUrl(), '', Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA));

        $cache = BP . DS . $media . 'catalog' . DS . 'product' . DS .'cache' . DS;
