...
class Magentothem_Banner7_Block_Banner7 extends Mage_Core_Block_Template
{
	...
	public function getDataBanner7()
    {
		...
		$select = $read->select()
		   ->from($slideTable,array('banner7_id','title','link','description','image','image_mobile','order', 'store_id','status'))
	...
	public function getConfig($att) 
	{
		...
		if (isset($config['banner7_config'])) {
			$value = (isset($config['banner7_config'][$att])? ($config['banner7_config'][$att]) : '');
			return $value;
		} else {
