...
class MageWorx_Adminhtml_Block_Customoptions_Adminhtml_Catalog_Product_Edit_Tab_Options_Option extends MageWorx_Adminhtml_Block_Customoptions_Adminhtml_Catalog_Product_Edit_Tab_Options_Option_Abstract {
    ...
    public function getViewQty($qty) {
        $prefix = '';
        if (substr($qty, 0, 1)=='i') {
            $IGI = substr($qty, 1);
            $qty = $this->getViewIGI($IGI);
            $prefix = 'i';
        }
        return $prefix . $qty;
    }
        ...
        if (!$this->_values) {
            ...
            foreach ($optionsCollection as $option) {
                ...
                if ($helper->isStoreViewsEnabled() && $option->getStoreViews() != null) {
                    $value['store_views'] = $option->getStoreViews();
                }
                ...
                if ($option->getGroupByType()==Mage_Catalog_Model_Product_Option::OPTION_GROUP_SELECT) {
                    ...
                    foreach ($option->getValues() as $_value) {
                        ...
                        if (!$helper->isSkuQtyLinkingEnabled() || $helper->getProductIdBySku($_value->getSku())==0) {
                            ...
                            list($customoptionsQty, $backorders) = $helper->getCustomoptionsQty($_value->getCustomoptionsQty(), $_value->getSku(), 0, $_value->getExtra(), null, null, null, true);
                        }
                        
                        $customoptionsMinQty = $_value->getCustomoptionsMinQty();
                        $customoptionsMaxQty = $_value->getCustomoptionsMaxQty();
                        ...
                        $value['optionValues'][$i] = array(
                            ...
                            'customoptions_qty' => $helper->removeQtyZeroes($customoptionsQty),
                            'customoptions_min_qty' => $helper->removeQtyZeroes($customoptionsMinQty),
                            'customoptions_max_qty' => $helper->removeQtyZeroes($customoptionsMaxQty),
                            'customoptions_qty_disabled' => ($helper->isSkuQtyLinkingEnabled() && $helper->getProductIdBySku($_value->getSku())?'disabled="disabled"':''),
                            'extra' => $this->getViewQty($_value->getExtra()),
                            'extra_disabled' => ($helper->isSkuQtyLinkingEnabled() && $helper->getProductIdBySku($_value->getSku())?'disabled="disabled"':''),
                        ...
                        if ($this->getProduct()->getStoreId() != '0') {
                            ...
                            $value['optionValues'][$i]['checkboxScopeDescription'] = $this->getCheckboxScopeHtml($_value->getOptionId(), 'description', is_null($_value->getStoreDescription()), $_value->getOptionTypeId());
                            $value['optionValues'][$i]['scopeDescriptionDisabled'] = is_null($_value->getStoreDescription()) ? 'disabled' : null;
    ...
    public function getStoreViewsMultiselectHtml()
    {
        $storeViews = Mage::getSingleton('adminhtml/system_store')->getStoreValuesForForm();

        $select = $this->getLayout()->createBlock('adminhtml/html_select')
            ->setData(array(
                'id' => $this->getFieldId() . '_{{id}}_store_views',
                'class' => 'select multiselect',
            ))
            ->setExtraParams('multiple="multiple" size="10"')
            ->setName($this->getFieldName() . '[{{id}}][store_views][]')
            ->setOptions($storeViews);
