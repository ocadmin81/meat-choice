...
class MageWorx_CustomOptions_Model_Observer {
        ...
        foreach ($options as $optionId => $option) {
            ...
            if ($helper->isInventoryEnabled()) {
                ...
                foreach ($option as $optionTypeId) {
                    ...
                    list($customoptionsQty, $backorders) = $helper->getCustomoptionsQty(isset($row['customoptions_qty'])?$row['customoptions_qty']:'', isset($row['sku'])?$row['sku']:'', $quoteItem->getProductId(), isset($row['extra'])?$row['extra']:'', $value, $quoteItem->getId(), $quote, true);
    ...
    public function insertNewOrderItem($sku, $optionType, $orderItem, $optionModel, $productOptions, $store, $optionId, $optionTypeId, $connection, $tablePrefix, $reduce, $orderTotalQtyOrdered, $customoptionsIsOnetime, $product, $isFirstGroupedInsert) {
        ...
        if (isset($productOptions['info_buyRequest']['options_'.$optionId.'_qty'])) {
            $optionQty = $helper->typifyQty($product, $productOptions['info_buyRequest']['options_'.$optionId.'_qty']);
        } elseif (isset($productOptions['info_buyRequest']['options_'.$optionId.'_'.$optionTypeId.'_qty'])) {
            $optionQty = $helper->typifyQty($product, $productOptions['info_buyRequest']['options_'.$optionId.'_'.$optionTypeId.'_qty']);
        } else {
            $optionQty = 1;
        }
        $optionTotalQty = ($customoptionsIsOnetime?$optionQty:$optionQty*$qty);
        ...
        if ($options) {
            ...
            foreach ($options as $optionId => $option) {
                ...
                foreach ($option as $optionTypeId) {
                    ...
                    $optionQty = $helper->getPostCustomoptionQty($quoteItem->getProduct(), $productOption, $postValueModel, $post);
