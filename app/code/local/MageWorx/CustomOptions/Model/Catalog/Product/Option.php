...
class MageWorx_CustomOptions_Model_Catalog_Product_Option extends Mage_Catalog_Model_Product_Option {
    ...
    public function _prepareOptions($options, $templateId, $groupIsActive = 2) {
        ...
        foreach ($options as $key=>$op) {                
            ...
            $op['store_views'] = isset($op['store_views']) ? implode(',', $op['store_views']) : '';
            ...
            if (!isset($op['max_characters'])) $op['max_characters'] = null;
            if (!isset($op['file_extension'])) $op['file_extension'] = null;
            if (!isset($op['image_size_x'])) $op['image_size_x'] = 0;
            if (!isset($op['image_size_y'])) $op['image_size_y'] = 0;
            ...
            if ($this->getGroupByType($op['type'])==self::OPTION_GROUP_SELECT) {
                if (isset($op['values']) && is_array($op['values'])) {
                    ...
                    foreach ($op['values'] as $k=>$value) {
                        ...
                        if (!isset($value['customoptions_qty'])) $value['customoptions_qty'] = NULL;
                        if (substr($value['extra'], 0, 1)=='i') {
                            $value['extra'] = 'i' . (($templateId * 65535) + intval(substr($value['extra'], 1)));
                        }
                        ...
                        if (!isset($value['customoptions_min_qty'])) $value['customoptions_min_qty'] = NULL;
                        if (!isset($value['customoptions_max_qty'])) $value['customoptions_max_qty'] = NULL;
...
//                exit;
    ...
    public function saveOption($productId, $option, $optionId = 0, $storeId = 0, $type = '') {
        ...
        if ($storeId==0) {
            ...
            if (isset($option['store_views'])) $optionData['store_views'] = $option['store_views'];
    ...
    public function saveOptionValue($optionId, $value, $prevValue, $newValue, $optionTypeId = 0, $storeId = 0) {
        ...
        if ($storeId==0) {
            ...
            if (isset($value['extra'])) $optionValue['extra'] = $value['extra'];
        ...
        if (isset($value['description']) && $value['description']!='') {
            ...
            $select = $connection->select()->from($tablePrefix . 'custom_options_option_type_description', array('description'))->where('option_type_id = '.$optionTypeId.' AND `store_id` = '.$storeId);
            $updateFlag = $connection->fetchOne($select);
            
            if ($value['description']!==$updateFlag) {
                if ($updateFlag) {
                    $connection->update($tablePrefix . 'custom_options_option_type_description', $optionTypeDescription, 'option_type_id = '.$optionTypeId.' AND `store_id` = '.$storeId);
                } else {
                    $connection->insert($tablePrefix . 'custom_options_option_type_description', $optionTypeDescription);
                }
            }    
        ...
        foreach ($this->getOptions() as $option) {
            ...
            if ($this->getData('is_delete')=='1') {
                ...
                if ($this->getGroupByType($this->getType())!=Mage_Catalog_Model_Product_Option::OPTION_GROUP_SELECT) {
                    ...
                    if (is_null($this->getTitle())) $this->setTitle('price by sku');
                }
            ...
            foreach ($this->getData('values') as $key => $value) {
                if (isset($value['option_type_id'])) {
                    ...
                    if (isset($value['customoptions_qty']) && (!$helper->isSkuQtyLinkingEnabled() || $helper->getProductIdBySku($value['sku'])==0)) {
                        ...
                        if (!is_numeric($customoptionsQty)) $customoptionsQty = NULL;
                    }
                    ...
                    if (isset($value['extra']) && (!$helper->isSkuQtyLinkingEnabled() || $helper->getProductIdBySku($value['sku'])==0)) {
                        ...
                        if (substr($extra, 0, 1)=='i') $extra = $this->decodeViewIGI($extra);
                    }
                    
                    $customoptionsMinQty = NULL;
                    if (isset($value['customoptions_min_qty'])) {
                        $customoptionsMinQty = $value['customoptions_min_qty'];
                        if (!is_numeric($customoptionsMinQty)) $customoptionsMinQty = NULL;
                    }
                    
                    $customoptionsMaxQty = NULL;
                    if (isset($value['customoptions_max_qty'])) {
                        $customoptionsMaxQty = $value['customoptions_max_qty'];
                        if (!is_numeric($customoptionsMaxQty)) $customoptionsMaxQty = NULL;
                    }
