    ...
    public function getValuesHtml() {
        ...
        if ($_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN || $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_MULTIPLE
            ...
            if ($_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN || $_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_SWATCH) {
                $select->setName('options[' . $_option->getid() . ']')->addOption('', $_option->getTitle());
            } else {
...
//                         </span>';
                    ...
                    $selectHtml .= 
                        '<span class="radio-checkbox-label">'.
                            '<label class="radio-checkbox-label-'. $cssVariant .'" onclick="if ($(Event.element(event)).hasClassName(\'qty\')) return false">' . 
                                ...
                                '<div class="label-qty"><b>'.$helper->getDefaultOptionQtyLabel().'</b> <input '.$qtyInputType.' class="qty'. ($checked?' validate-greater-than-zero':'') .'" value="'.$optionValueQty.'" maxlength="12" id="options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty" name="options_'.$_option->getId().'_'.$_value->getOptionTypeId().'_qty" pattern="\d*" onchange="'. $optionJs .'" onKeyPress="if(event.keyCode==13){'. $optionJs .'}" '.($checked?$disabled:'disabled').'></div>'.
                            '</label>';
                } elseif ($imagesHtml) {
                    $selectHtml .=                        
                        '<span class="radio-checkbox-label">'.
                    ...
                    $selectHtml .=
                        '<input ' . $disabled . ' ' . $checked . ' type="' . $type . '" font-src="' .$_value->getDescription() . '" class="' . $class . ' ' . $require . ' product-custom-option" onclick="'.$onclick.'" name="options[' . $_option->getId() . ']' . $arraySign . '" id="options_' . $_option->getId() . '_' . $count . '" value="' . $_value->getOptionTypeId() . '" />'.
                        '<span class="label"><label for="options_' . $_option->getId() . '_' . $count . '">' . $_value->getTitle() . ' ' . $priceStr . $qty . '</label>';
                }

                if ($_value->getDescription() && $helper->isOptionVariationDescriptionEnabled() && $_option->getType()!=Mage_Catalog_Model_Product_Option::OPTION_TYPE_RADIO) {
