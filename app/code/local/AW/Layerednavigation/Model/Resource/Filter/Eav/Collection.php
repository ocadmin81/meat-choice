...
class AW_Layerednavigation_Model_Resource_Filter_Eav_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
     ...
     * @return AW_Layerednavigation_Model_Resource_Filter_Eav_Collection
     */
    public function getAttributeCollectionByStoreId($filterId, $storeId = Mage_Catalog_Model_Abstract::DEFAULT_STORE_ID)
    {
        $this->getSelect()
            ->reset(Zend_Db_Select::COLUMNS)
            ->columns(array('eav_id', 'filter_id', 'name'))
            ->where('main_table.filter_id = ?', $filterId)
            ->where('main_table.store_id = ?', Mage_Catalog_Model_Abstract::DEFAULT_STORE_ID)
            ->joinLeft(
                array('current_store_attribute' => $this->getTable('aw_layerednavigation/filter_eav')),
                "main_table.name = current_store_attribute.name
                AND main_table.filter_id = current_store_attribute.filter_id
