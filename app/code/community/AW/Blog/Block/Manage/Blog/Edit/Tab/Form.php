...
class AW_Blog_Block_Manage_Blog_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
    ...
    protected function _prepareForm() {
		...
		$fieldset->addField('banner', 'image', array(
			'label'     => Mage::helper('blog')->__('Main Banner'),
			'required'  => false,
			'name'      => 'banner',
		));	
        /**
		...
		$fieldset->addField('in_home', 'select', array(
            'label' => Mage::helper('blog')->__('Show in home'),
            'name' => 'in_home',
            'values' => array(
                array(
                    'value' => 0,
        ...
        $fieldset->addField('related_product', 'text', array(
            'label' => Mage::helper('blog')->__('Related Product'),
            'required' => false,
            'name' => 'related_product',
        ));
		
		$fieldset->addField('additional_recipes', 'text', array(
            'label' => Mage::helper('blog')->__('Additional Recipes'),
            'required' => false,
            'name' => 'additional_recipes',
        ));		
		
        $fieldset->addField('making_time', 'text', array(
            'label' => Mage::helper('blog')->__('Making Time'),
            'required' => false,
            'name' => 'making_time',
        ));
		
        $fieldset->addField('difficulty', 'select', array(
            'label' => Mage::helper('blog')->__('Difficulty'),
            'name' => 'difficulty',
            'values' => array(
                array(
                    'value' => '',
                ...
                array(
                    'value' => 'Easy',
                    'label' => Mage::helper('blog')->__('Easy'),
                ),
                array(
                    'value' => 'Medium',
				...
				array(
                    'value' => 'Hard',
                    'label' => Mage::helper('blog')->__('Hard'),
                ),
            ),            
        ));
		
		$fieldset->addField('cooking_type', 'select', array(
            'label' => Mage::helper('blog')->__('Cookin Type'),
            'name' => 'cooking_type',
            'values' => array(
                array(
                    'value' => '',
                ...
                array(
                    'value' => 'Frying',
                    'label' => Mage::helper('blog')->__('Frying'),
                ),
				array(
                    'value' => 'Grilled',
                    'label' => Mage::helper('blog')->__('Grilled'),
                ),
                array(
                    'value' => 'Cooking',
                    'label' => Mage::helper('blog')->__('Cooking'),
                ),
            ),            
        ));
		...
		$fieldset->addField('post_video', 'editor', array(
            'name' => 'post_video',
            'label' => Mage::helper('blog')->__('Video'),
            'title' => Mage::helper('blog')->__('Video'),
            'style' => 'width:700px; height:500px;',
            'config' => $config
        ));
