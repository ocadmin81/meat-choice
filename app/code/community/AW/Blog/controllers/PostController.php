...
class AW_Blog_PostController extends Mage_Core_Controller_Front_Action {
	...
	public function loadAction(){
		...
		$cat_posts = explode(",",$_POST['postsUse']);
		$cat_id = $_POST['catId'];
		$html = '';
		$html .= '<div class="owl-carousel owl-theme" id="carousel-'.$cat_id.'"">';
		foreach ($cat_posts as $post){
			$sql2 = $connection->query("select title,thumbnail,making_time,cooking_type,post_id,identifier from $tblname1 where $post=post_id");
			$row2 = $sql2->fetch();
			$html .= '<div class="item post-'.$post.'">' ;
				$html .= '<div class="post-img">';
					$html .= '<a href="'.Mage::getUrl('blog').$row2['identifier'].'"><img src="'.Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).$row2['thumbnail'].'" class="main_image" /></a>';
					$html .= '<div class="post-details">';
						$html .= '<ul>';
							if($row2['cooking_type'] != ''){
								$html .= '<li class="cooking_type">'.$this->__('Cooking Type').': '.$this->__($row2['cooking_type']).'</li>';
							}
							if($row2['making_time'] != ''){
								$html .= '<li class="making_time">'.$this->__('Making Time').': '.$this->__($row2['making_time']).' '.$this->__('minutes').'</li>';
							}							
						$html .= '</ul>';
					$html .= '</div>';
				$html .= '</div>';
				$html .= '<div class="name">';
					$html .= '<a href="'.Mage::getUrl('blog').$row2['identifier'].'">'.$row2['title'].'</a>';
				$html .= '</div>';
			$html .= '</div>';
		}
		$html .= '</div>';		
