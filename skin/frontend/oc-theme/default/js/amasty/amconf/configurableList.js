                    ...
                    if (options[i].color) {
                        ...
                        if(typeof confData[this.config.productId] != 'undefined' && confData[this.config.productId].getData(keyOpt, 'not_is_in_stock')){
                            ...
                            image.onload = function(){
                                ...
                                if(hrElement){
                                    hrElement.style.width =   Math.sqrt(maxW*maxW + maxH*maxH) + 1 + 'px';
                                    hrElement.style.top =   maxH/2 + 1  + 'px';
                                    hrElement.style.left =   -(Math.sqrt(maxW*maxW + maxH*maxH) - maxH)/2 + 2 + 'px';
                                }
                            };
                            hr.observe('mouseover', this.configureHr.bind(hr.hr));

                        }
                        image.alt = options[i].label;
                        image.title = options[i].label;
                        image.observe('mouseover', this.configureImage.bind(this));
